...
def postproces_chapter_data(
    ...
    """
    if module == 1:
        return postprocess_chapter1_data(data)
    elif module == 2:
        return postprocess_chapter2_data(data)
    elif module == 3:
        return postprocess_chapter3_data(data)
    elif module == 4:
...
def postprocess_data(
    ...
    for i, data in cleaned_raw_data.items():
        # 剔除冗余数据
        processed_chapter_data = postproces_chapter_data(i, data)
        # 增加省份销售人员数量
        if i == 1 or i == 3:
            processed_chapter_data["销售所属省区销售总人数"] = province_sales_count.get(sale_province, "无")
