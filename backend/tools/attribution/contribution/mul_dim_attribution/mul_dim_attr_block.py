...
from .mul_arg_model import MulDimArgModel

class MulDimAttribution:
    def __init__(
            self,
            arg_model4llm: Type[BaseModel],  # LLM输入参数模型
            arg_model: Type[BaseModel],  # 内部输入参数模型
            args_model_creator: MulDimArgModel,
            relation_type: AttributionRelationType = AttributionRelationType.MULTIPLICATIVE  # 维度关系类型
    ):
        """
        初始化函数
        Args:
            ...
            arg_model: 内部输入参数模型
            args_model_creator: 参数模型构建器（具备维度归因块构建功能）
            relation_type: 维度关系类型（默认乘法关系）
        Returns:
            None
        """
    ...
    def attribution_function(
        ...
        Returns:
            Tuple[DimensionAttributionOutput, List[str]]: 维度归因分析结果
        """
    ...
    def wrapped_attribution_function(
        ...
        """
        包装函数
        将LLM输入转换为内部输入模型，然后进行归因分析
        Args:
            kwargs: 关键字参数（LLM输入参数）
        Returns:
            Tuple[DimensionAttributionOutput, List[str]]: 维度归因分析结果
        """
