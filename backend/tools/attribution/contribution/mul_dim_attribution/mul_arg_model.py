...
"""参数模型构建器"""
class MulDimArgModel:
    """
    乘法维度参数模型构建器
    根据指标配置动态创建输入参数模型
    支持乘法关系：A = B * C（仅支持2个子指标）
    """
    ...
    def __init__(
            ...
            dimension_name: str,
            child_indicator_name_1: str,
            child_indicator_name_2: str,
            parent_key: Optional[str] = None,
            dimension_key: Optional[str] = None,
            child_key_1: Optional[str] = None,
            child_key_2: Optional[str] = None
    ):
        """
        :param parent_indicator_name: 父指标名称（如 "收入"）
        :param dimension_name: 维度名称（如 "价格销量维度"）
        :param child_indicator_name_1: 第一个子指标名称（如 "单价"）
        :param child_indicator_name_2: 第二个子指标名称（如 "销量"）
        :param parent_key: 父指标键名（可选，如果不提供则自动从parent_indicator_name生成）
        :param dimension_key: 维度键名（可选，如果不提供则自动从dimension_name生成）
        :param child_key_1: 第一个子指标键名（可选，如果不提供则自动从child_indicator_name_1生成）
        :param child_key_2: 第二个子指标键名（可选，如果不提供则自动从child_indicator_name_2生成）

        使用示例：
            # 创建乘法归因模型：收入 = 单价 * 销量
            model = MulDimArgModel(
                parent_indicator_name="收入",
                dimension_name="价格销量维度",
                child_indicator_name_1="单价",
                child_indicator_name_2="销量"
            )
        """
        self.parent_indicator_name = parent_indicator_name
        self.dimension_name = dimension_name
        self.child_indicator_name_1 = child_indicator_name_1
        self.child_indicator_name_2 = child_indicator_name_2
        self.parent_key = parent_key
        self.dimension_key = dimension_key
        self.child_key_1 = child_key_1
        self.child_key_2 = child_key_2
    ...
    def _create_indicators_config(self) -> Dict[str, Any]:
        ...
        返回格式：
        {
            "parent_indicator": {"name": "收入", "key": "shou_ru"},
            "dimension_name": "价格销量维度",
            "child_indicators": [
                {"name": "单价", "key": "dan_jia"},
                {"name": "销量", "key": "xiao_liang"}
            ]
        }
        """
        # 自动生成或使用提供的key
        ...
        if self.child_key_1 is None:
            self.child_key_1 = name_to_snake_key(self.child_indicator_name_1)

        if self.child_key_2 is None:
            self.child_key_2 = name_to_snake_key(self.child_indicator_name_2)
        ...
        config = {
            ...
            "child_indicators": [
                {
                    "name": self.child_indicator_name_1,
                    "key": self.child_key_1
                },
                {
                    "name": self.child_indicator_name_2,
                    "key": self.child_key_2
                }
            ]
        }
    ...
    def create_dimension_attribution_block(
            ...
            relation_type: AttributionRelationType = AttributionRelationType.MULTIPLICATIVE
    ) -> DimensionAttributionBlock:
        """
        ...
        :param relation_type: 维度关系类型（默认乘法关系）
        ...
        使用示例：
            ...
            llm_input = llm_input_class(
                ...
                shou_ru_prior=900.0,
                dan_jia_current=10.0,
                dan_jia_prior=9.0,
                xiao_liang_current=100.0,
                xiao_liang_prior=100.0
            )
            ...
            dim_block = model.create_dimension_attribution_block(
                ...
                relation_type=AttributionRelationType.MULTIPLICATIVE
            )
        ...
        # 提取子指标的 IndicatorSpec 列表（乘法关系固定为2个）
        sub_indicators = []
        for child_info in self.config["child_indicators"]:
        ...
        if len(sub_indicators) != 2:
            raise ValueError(f"乘法关系归因需要恰好2个子指标，但得到 {len(sub_indicators)} 个")
...
if __name__ == "__main__":
    ModelCreator = MulDimArgModel(
        parent_indicator_name="收入",
        dimension_name="客户维度",
        child_indicator_name_1="客户数",
        child_indicator_name_2="客均收入"
    )
