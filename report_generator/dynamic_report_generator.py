...
class DynamicReportGenerator:
    ...
    async def _fetch_all_chapter_data(self):
        ...
        # 批量获取数据
        data_list = await fetch_raw_data_batch(requests, concurrent_limit=5)

        # 存储数据
        for ch, data in zip(chapters_needing_data, data_list):
            self.chapter_data[ch.chapter_id] = data
    ...
    async def _process_data(self):
        ...
        for chapter_id, data in self.chapter_data.items():
            ...
            # 数据后处理
            processed_data = postproces_chapter_data(module=chapter_id, data=data)
    ...
    async def _wrap_to_html(self, md_path: Path) -> Path:
        ...
        decorator = ReportDecorator(
            ...
            html_report_folder=self.progress_report_dir,
...
if __name__ == "__main__":
    ...
    async def test_dynamic_generator():
        ...
        print("\n" + "=" * 50)
        print("创建模型实例...")
        llm = create_model(model_type="local")
